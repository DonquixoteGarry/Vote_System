...
```py
# 建立数据集格式
# root:源数据根目录,transform:数据格式转换函数
# 此处compose依次连接四个函数,即 图像长宽重整、中心裁剪、张量化、标准归一化(归至[0,1],均值,标准差,协方差)
# image_size上文规定64
...
# 建立数据装载器格式,参数(源数据集,批量规模,是否打乱,工作进程数)
...
# 展示输出图片
# iter(dataloader)作为数据装载器里的迭代器,指向一个两对象列表:(张量化图片,零一真伪概率标签)
# real_batch即
